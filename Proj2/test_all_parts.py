...
################################################################################
'test for sequential with CrossEntropy'
if False:
    ...
    for i in range(number_tests):
        ...
        optim_test = SGD(test.param(),lr, momentum = True, mu = 0.2)

        ref = Model()
        optim_ref = optim.SGD(ref.parameters(), lr, momentum=0.2)
...
################################################################################
if True:
    print('test for Sequential for MSE')
    ...
    #building the summy datasets
    x_test = torch.empty(number_samples,n1).normal_(10)
    y_test = torch.empty(number_samples, n2).random_(5)
    x_ref = x_test.clone()
    y_ref = y_test.clone()
    x_ref.requires_grad = True
    ...
    for i in range(number_tests):
        ...
        lr = 1e-1
        test = Sequential(Linear(n1,100),Relu(),Linear(100,100),Relu(),Linear(100,n2))
        ...
        layer1_test = test.layers[0]
        layer2_test = test.layers[-3]
        layer3_test = test.layers[-1]
        layer1_test.weights.value = ref.Fc1.weight.data.clone()
        layer1_test.bias.value = ref.Fc1.bias.data.clone()
        layer2_test.weights.value = ref.Fc2.weight.data.clone()
        layer2_test.bias.value = ref.Fc2.bias.data.clone()
        layer3_test.weights.value = ref.Fc3.weight.data.clone()
        layer3_test.bias.value = ref.Fc3.bias.data.clone()
        ...
        for e in range(10):
            ...
            output_ref = ref(x_ref)
            output_test = test.forward(x_test)
            if (output_test != output_test).sum() > 0:
                print("error output {},{}".format(i,e))
                break
            mean_error += torch.max(output_test-output_ref)
            ...
            #comparing the losses again (in case of)
            loss_test = criterion_test.forward(output_test,y_test)
            if (loss_test!=loss_test).sum()>0:
                print("error loss  {},{}".format(i,e))
            loss_ref = criterion_ref(output_ref,y_ref)
            mean_error_loss += torch.max(loss_test-loss_ref)
            print(mean_error_loss/((i+1)*(e+1)))
            ...
            if (grad_w_test_layer2 !=grad_w_test_layer2).sum()>0:
                print("error grad  {},{}".format(i,e))
